...
public class CarDealershipSimulator {
	 ...
	 * Reads from "cars.txt" file and creates car objects and adds them to the
	 * fileCars car list.
	 * 
	...
	public static ArrayList<Car> getCarsFromFile() throws FileNotFoundException {
		...
		// Initializes fileCars ArrayList to hold the list of Car objects being made
		// from the cars.txt file
		...
		while (fileIn.hasNextLine()) {
			...
			// If there are 8 entries in a line, create a Car object and add it to the
			// fileCars ArrayList
			if (splitLine.length == 8) {
				fileCars.add(new Car(tempManufacturer, tempColor, 300, 4, tempELECTRIC_ENGINE, tempGAS_ENGINE,
						tempModel, tempMaxRange, tempSafetyRating, tempAWD, tempPrice));
			}
			// Else, create an ElectricCar object and add it to the fileCars ArrayList
			else {
				int tempChargeTime = Integer.parseInt(splitLine[8]);
				fileCars.add(new ElectricCar(tempManufacturer, tempColor, 300, 4, tempELECTRIC_ENGINE, tempGAS_ENGINE,
						tempModel, tempMaxRange, tempSafetyRating, tempAWD, tempPrice, tempChargeTime, "Lithium-Ion"));
	...
	public static void main(String[] args) {
		// Initiliazes fileCars variable to hold the Car objects read from the cars.txt
		// file
		ArrayList<Car> fileCars = new ArrayList<Car>();
		...
		while (mainScanner.hasNextLine()) {
			...
			// Commands that are 3 characters or longer
				...
				// User input "BUY n", where n is an integer, removes car from dealership list,
				// prints cars information
				...
				// User input "ADD", adds array list of car objects to dealership object's array
				// list
				...
				// User input "SALES STATS", displays total sales, net total sales, average
				// sales per month,
				// net average sales per month, cars sold, cars returned, and the highest
				// month(s) of sales
					...
					// User input "SALES m", where m is an integer from 0-11, displays all
					// transactions that
					...
					if (newline.substring(0, 5).equals("SALES")) {
						try {
							...
							if (month < 0 || month > 11) {
								...
								System.out.println(
										dealership.dealershipAccountingSystem.displayMonthTransactionList(month));
							}
						} catch (NullPointerException e) {
							System.out.println("Please enter a valid index.");
						} catch (NumberFormatException e) {
