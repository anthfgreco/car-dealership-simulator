...
public class AccountingSystem {
    ...
    /**
     * Takes transaction parameters to create a transaction object and adds it to the transactionList array list.
    ...
    public String add(Calendar date, Car car, String salesPerson, String type, double salePrice){
        ...
        return String.format("\n%s                  %s  %s  %s  %s  %s   %s   %s   %s %s  %s     %s    %s   %s\n%s", "#", "Date", "Type", "Sales Member", "VIN", "Manufacturer", "Color", "Model", "Range", "Safety", "Powertrain", "Price",  "Chg. Time", "Battery Type", newTransaction.display());       
    }
    /**
     * Takes a transaction ID and searches the transactionList array list for a matching ID.
     * @param id transaction ID to search for
     * @return tempTransaction; if the parameter ID matches an ID found in the transactionList
    ...
    public String displayTransactionList(){
        String result = "";
        System.out.printf("\n%s                  %s  %s  %s  %s  %s   %s   %s   %s %s  %s     %s    %s   %s", "#", "Date", "Type", "Sales Member", "VIN", "Manufacturer", "Color", "Model", "Range", "Safety", "Powertrain", "Price",  "Chg. Time", "Battery Type");       
        for (Transaction tempTransaction : transactionList) {
    ...
    /**
     * Takes a transaction ID and searches through the transactionList array list if the car was already returned.
     * @param transactionID transaction ID to search for
     * @return true; if the transaction type is "RET" OR if the car object for the transaction ID has been returned before
     *         false; otherwise
     */
    ...
    /**
     * Calculates which sales members have the most number of sales and returns a string of the result.
     * This method uses a map to map sales members to number of sales, then uses Collections.max to find the
     * max value of the map and adds every key (sales member) that has this max value to the resulting string.
     * This means if every sales member has an equal amount of sales, every sales member is a top sales member.
    ...
    /**
     * Similar to displayTransactionList, displays the account system object for the given month in a string table format.
     * @param m month integer from 0-11
     * @return the string representation of the transactions held by the accounting system object for month m
     */
    ...
    /**
     * Calculates which months have the most number of sales and returns a string of the result.
     * This method uses a map to map months to number of sales, then uses Collections.max to find the
     * max value of the map and adds every key (month) that has this max value to the resulting string.
     * This means if every month has an equal amount of sales, every month is also a top month.
    ...
    /**
     * Calculates stats based on transactions in the transactionList array list and returns them in string format.
     * @return string representation of total sales, net total sales, average sales per month, net 
     *         average sales per month, cars sold, cars returned, and the highest month(s) of sales        
     */
    public String displayStats(){
        ...
        return String.format("\nTotal Sales: $%s\nTotal Net Sales: $%s\nAverage Sales Per Month: $%s\nAverage Net Sales Per Month: $%s\nCars Sold: %s\nTotal Returned: %s\nBest Month(s): %s\n", totalSales, totalSalesNet, averageSalesPerMonth, averageNetSalesPerMonth,carsSold, carsReturned, highestSalesMonths);
    }
    /**
